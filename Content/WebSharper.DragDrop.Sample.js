...
function Main(){
  ...
  const b=(this_1.h.push(new AfterRenderQ("boxinit", () => {
    setupDragAndDrop();
  })),this_1);
...
function setupDragAndDrop(){
  const dragBox=globalThis.document.getElementById("dragBox");
  const dropZone=globalThis.document.getElementById("dropZone");
  dragBox.addEventListener("dragstart", (event) => event.dataTransfer.setData("text/plain", "Dropped!"));
  dropZone.addEventListener("dragover", (event) => event.preventDefault());
  dropZone.addEventListener("drop", (event) => {
    event.preventDefault();
    dropZone.textContent=event.dataTransfer.getData("text/plain");
    return dropZone.classList.add("drop-element");
  });
}
